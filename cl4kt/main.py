...
def main(config):
    ...
    if model_name == "cl4kt":
        df2 = pd.read_csv(os.path.join(os.path.join(dataset_path, data_name), "preprocessed_test_df.csv"), sep="\t")
    ...
    for fold, (train_ids, valid_ids) in enumerate(kfold.split(users)):
        if model_name == "akt":
...
if __name__ == "__main__":
    ...
    base_cfg_file = PathManager.open("/workspace/cl4kt/configs/example.yaml", "r")
